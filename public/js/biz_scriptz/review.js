show_tabs();
function get_bind_biz_item(){
    hide_tabs();
    tbl_id = $('#tb_item_tbl_id').val();
    data_type=$('#tb_item_data_type').val();
    $.ajax({
        type: "GET",
        url: "/cloud/crud/get/"+data_type+"/"+tbl_id,
        enctype: 'multipart/form-data',
        data: {},
        success: function(data){
            console.log(data);
            if(data.helper){
                show_tabs();
                bind_photo_setting(data.helper.item);
                init_item_note(data.helper.item.note);
                bind_biz_prop_obj(data);
                $('#tb_item_edit_name').val(data.helper.item.name);
                $('#tb_item_edit_location').val(data.helper.item.location);
                $('#tb_item_edit_position').val(data.helper.item.position);
                $('#tb_item_edit_comment').val(data.helper.item.comment);
            }else{
                alert('Please make an selection');
            }
        }
    });
}
//9_item_copy
$("#tbl_list").on("click", ".link_item_copy", function(){
    tbl_id = $(this).attr('tbl_id');
    $.ajax({
        type: "POST",
        url: "/comment/copy_comment/"+tbl_id,
        enctype: 'multipart/form-data',
        data: {},
        success: function(data){
            window.location.reload();
        }
    });
});
//9_item_submit
//9_item_update
$('#btn_item_submit').click(function(){
    tbl_id = $('#tb_item_tbl_id').val();
    data_type = $('#tb_item_data_type').val();
    var obj={};
    obj.name=$('#tb_item_edit_name').val();
    obj.comment=$('#tb_item_edit_comment').val();
    obj.location=$('#tb_item_edit_location').val();
    obj.position=$('#tb_item_edit_position').val();
    obj.photofilename=$('#tb_item_photofilename').val();
    hide_tabs();
    $.ajax({
        type: "POST",
        url: "/cloud/crud/update/"+data_type+"/"+tbl_id,
        enctype: 'multipart/form-data',
        data: obj,
        success: function(data){
            $("#div_validation_message").show().delay(2000).fadeOut();
            $("#h_item_edit_name").html(obj.name);
            show_tabs();
        }
    });
});

//9_item_add_submit
$('#btn_item_add_submit').click(function(event){
    var obj={};
    tbl_id=0;
    data_type=$('#tb_item_data_type').val();
    list_total=$('#tb_item_list_count').val();
    obj.name=$('#tb_item_add_title').val();
    obj.visible='true';
    obj.order=parseInt(list_total)+1;
    if(obj.name.length<=0){
        Swal.fire("Error",'Name must contain at least 3 characters',"error");
    }else{
        obj.title_url = obj.name.replace(/[^a-z0-9]+/ig, "_").toLowerCase();
        $.ajax({
            type: "POST",
            url: "/cloud/crud/update/"+data_type+"/"+tbl_id,
            enctype: 'multipart/form-data',
            data: obj,
            success: function(data){
                if(data.helper.error){
                    Swal.fire("Error",data.helper.error,"error");
                }else{ window.location.reload();
                }
            }
        });
    }
    //return false;
});
//9_item_edit
$("#tbl_list").on("click", ".link_item_edit", function(){
    tbl_id = $(this).attr('tbl_id');
    window.location='/comment/edit_comment/'+tbl_id;
});
//9_delete
$("#tbl_list").on("click", ".link_item_delete", function(){
    if (confirm("Are you sure?")) {
        tbl_id = $(this).attr('tbl_id');
        data_type = $(this).attr('data_type');
        $.ajax({
            type: "POST",
            url: "/item/delete_item/"+data_type +'/'+tbl_id,
         enctype: 'multipart/form-data',
            data: {},
            success: function(data){
                 if(data.helper.error){
                    alert(data.helper.error);
                }else{
                    window.location.reload();
                }
         }
        });
    }
});
